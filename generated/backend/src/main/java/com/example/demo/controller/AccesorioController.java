package com.example.demo.controller;

import com.example.demo.model.Accesorio;
import com.example.demo.service.AccesorioService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.*;
import org.springframework.web.bind.annotation.*;
import java.util.*;

/**
 * Controlador REST generado automáticamente para la entidad Accesorio.
 * Gestiona operaciones CRUD básicas y admite DTOs si existen.
 */
@RestController
@RequestMapping("/api/accesorio")
@CrossOrigin(origins = "*")
public class AccesorioController {

    @Autowired
    private AccesorioService service;


    // CREATE
    @PostMapping("/guardar")
    public ResponseEntity<Accesorio> guardar(
            @RequestBody Accesorio data) {

        return ResponseEntity.status(HttpStatus.CREATED).body(service.save(data));
    }

    // READ ALL
    @GetMapping("/listar")
    public ResponseEntity<List<Accesorio>> listar() {
        List<Accesorio> list = service.findAll();

        return ResponseEntity.ok(list);
    }

    // READ BY ID
    @GetMapping("/{id}")
    public ResponseEntity<Accesorio> obtenerPorId(@PathVariable Long id) {
        Optional<Accesorio> data = service.findById(id);

        if (data.isEmpty()) return ResponseEntity.notFound().build();

        return ResponseEntity.ok(data.get());
    }

    // UPDATE
    @PutMapping("/actualizar/{id}")
    public ResponseEntity<Accesorio> actualizar(
            @PathVariable Long id,
            @RequestBody Accesorio data) {

        Accesorio entity = data;

        // Asignar ID por reflexión para mantener consistencia
        try {
            entity.getClass().getMethod("setId", Long.class).invoke(entity, id);
        } catch (Exception ignored) {}

        Accesorio updated = service.save(entity);

        return ResponseEntity.ok(updated);
    }

    // DELETE
    @DeleteMapping("/eliminar/{id}")
    public ResponseEntity<Void> eliminar(@PathVariable Long id) {
        service.delete(id);
        return ResponseEntity.noContent().build();
    }

}
